...
describe('Message Controller', function () {
    ...
    describe('getMessages', function () {
        ...
        it('should return 404 if conversation not found', async function () {
            sinon.stub(Conversation, 'findOne').resolves(null);

            const response = await supertest(app).get('/getMessages/user2');

            expect(response.status).to.equal(404);
            expect(response.body).to.have.property('error', 'Conversation not found');
        });
    ...
    describe('getConversations', function () {
        it('should retrieve conversations excluding the current user', async function () {
            const mockConversations = [
                {
                    _id: 'conversationId',
                    participants: [{ _id: 'user2', username: 'User Two', profilePic: 'pic.jpg' }]
                }
            ];
